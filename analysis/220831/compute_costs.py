...
def compute_costs(df):
    ...
    df["cost_facility"] = df["cases_treated"] * (cost_dict["facility_treatment"] + cost_dict["RDT"] + cost_dict["AL"])
    ...
    df["cost_mean"] = df["cost_IPTsc_commodities"] + df["cost_facility"] + df["cost_severe"] + \
                      df["cost_ITNs"] + df["cost_SMC_mean"] + df["cost_ivermectin"] + df["cost_primaquine"] + \
                      df["IPTsc_overhead_cost_mean"]

    df["cost_low_IPTsc_only"] = df["cost_IPTsc_commodities"] + df["cost_facility"] + df["cost_severe"] + \
                      df["cost_ITNs"] + df["cost_SMC_mean"] + df["cost_ivermectin"] + df["cost_primaquine"] + \
                      df["IPTsc_overhead_cost_low"]

    df["cost_high_IPTsc_only"] = df["cost_IPTsc_commodities"] + df["cost_facility"] + df["cost_severe"] + \
                      df["cost_ITNs"] + df["cost_SMC_mean"] + df["cost_ivermectin"] + df["cost_primaquine"] + \
                      df["IPTsc_overhead_cost_high"]

    df["cost_low"] = df["cost_IPTsc_commodities"] + df["cost_facility"] + df["cost_severe"] + \
                      df["cost_ITNs"] + df["cost_SMC_low"] + df["cost_ivermectin"] + df["cost_primaquine"] + \
                      df["IPTsc_overhead_cost_low"]

    df["cost_high"] = df["cost_IPTsc_commodities"] + df["cost_facility"] + df["cost_severe"] + \
